...
(function (global, factory) {
  ...
  function scott(values, min, max) {
    return Math.ceil(
      (max - min) /
        (3.5 * deviation(values) * Math.pow(count$1(values), -1 / 3))
    );
  }
  ...
  function ribbon(headRadius) {
    ...
    function ribbon() {
      ...
      if (ap > epsilon$4) {
        if (abs$2(sa1 - sa0) > ap * 2 + epsilon$4)
          sa1 > sa0 ? ((sa0 += ap), (sa1 -= ap)) : ((sa0 -= ap), (sa1 += ap));
        else sa0 = sa1 = (sa0 + sa1) / 2;
        if (abs$2(ta1 - ta0) > ap * 2 + epsilon$4)
  ...
  function longitude(point) {
    if (abs$1(point[0]) <= pi$1) return point[0];
    else return sign$1(point[0]) * (((abs$1(point[0]) + pi$1) % tau$1) - pi$1);
  }
  ...
  function quantize() {
    ...
    function scale(x) {
      return x != null && x <= x
        ? range[bisectRight(domain, x, 0, n)]
        : unknown;
    }
  ...
  function sequentialQuantile() {
    ...
    function scale(x) {
      if (x != null && !isNaN((x = +x)))
        return interpolator(
          (bisectRight(domain, x, 1) - 1) / (domain.length - 1)
        );
    }
  ...
  Basis.prototype = {
    ...
    lineEnd: function () {
      switch (this._point) {
        case 3:
          point$3(this, this._x1, this._y1); // proceed
        case 2:
